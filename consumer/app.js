...
function updateTimeDisplay() {
    const now = new Date();
    document.getElementById('timeDisplay').textContent = now.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false });
}

function updateGreeting() {
    document.getElementById('greetingText').textContent = getGreeting() + '，探索者';
}
...
function updateHomePage() {
    ...
    if (completedWeeks === 0 && state.currentWeek === 0) {
        ...
        heroTitle.textContent = `第 ${currentWeek} 周：${PM_DATA.weeks[currentWeek - 1].title}`;
        heroDesc.textContent = PM_DATA.weeks[currentWeek - 1].desc;
        heroProgress.style.display = 'flex';
        document.getElementById('pmProgressFill').style.width = `${(completedWeeks / 5) * 100}%`;
        document.getElementById('pmProgressText').textContent = `${completedWeeks}/5 周`;
...
function renderWeekList() {
    ...
    container.innerHTML = PM_DATA.weeks.map((week, i) => {
        ...
        let statusHtml = '';
        if (isCompleted) {
            statusHtml = '<span class="week-status done"><i class="fas fa-check-circle"></i></span>';
        } else if (isCurrent) {
        ...
        return `
            <div class="week-card ${isCurrent ? 'active' : ''} ${!isUnlocked ? 'locked' : ''} ${isCompleted ? 'completed' : ''}" 
                 onclick="${isUnlocked ? `startWeek(${i + 1})` : ''}">
                <div class="week-icon" style="background: ${week.color}; color: ${week.iconColor}">
    ...
    if (state.homework) {
        ...
        card.innerHTML = `
            <div class="homework-header">
                <div class="homework-icon"><i class="fas fa-${state.homework.icon}"></i></div>
            ...
            <div class="homework-progress">
                <div class="progress-bar"><div class="progress-fill" style="width: ${state.homeworkProgress}%"></div></div>
            </div>
            <p class="homework-days">已完成 ${Math.round(state.homeworkProgress / 20)}/5 天</p>
        ...
        if (step.input) {
            inputArea.innerHTML = `
                <div class="input-wrapper">
                    <input type="text" id="stepInput" placeholder="${step.input.placeholder || '输入...'}">
...
function getImprovementScore() {
    ...
    const pre = PM_DATA.state.psychlops.pre.Q1_Score ? calculatePsychlopsScore(PM_DATA.state.psychlops.pre) : 0;
...
function selectLikert(el, score, stepId) {
    document.querySelectorAll('.likert-item').forEach(i => i.classList.remove('selected'));
...
function renderOptions(config, stepId) {
    ...
    return `
        <div class="options-container" data-step="${stepId}" data-multi="${isMulti}">
            ${items.map(item => `
                <div class="option-card" data-value="${item.id}" onclick="selectOption(this)">
...
function renderActionButtons(actions) {
    return `
        <div class="action-buttons">
            ${actions.map(action => `
                <button class="btn btn-${action.type || 'primary'}" onclick="handleAction('${action.next}')">
                    ${action.text}
                </button>
            `).join('')}
        </div>
...
function handleAction(next) {
    // 处理多选确认
    const container = document.querySelector('.options-container[data-multi="true"]');
    if (container) {
        const selected = Array.from(container.querySelectorAll('.option-card.selected'))
...
function renderDragClassify() {
    ...
    const problemItems = PM_DATA.problemOptions.filter(p => problems.includes(p.id));
    
    // 如果没有选择，使用之前PSYCHLOPS的问题
    const allItems = problemItems.length > 0 ? problemItems : [
        { id: PM_DATA.state.psychlops.pre.Q1_Text, text: PM_DATA.state.psychlops.pre.Q1_Text },
        PM_DATA.state.psychlops.pre.Q2_Text ? { id: PM_DATA.state.psychlops.pre.Q2_Text, text: PM_DATA.state.psychlops.pre.Q2_Text } : null
    ].filter(Boolean);
...
function dropProblem(event, type) {
    ...
    if (dragItem) {
        const targetZone = document.getElementById(type === 'solvable' ? 'solvableItems' : 'unsolvableItems');
        ...
        if (!PM_DATA.state.unsolvableProblems) PM_DATA.state.unsolvableProblems = [];
    ...
    return `
        <div class="voice-input-container">
            <div class="voice-text-input">
                <input type="text" id="voiceTextInput" placeholder="${config.placeholder || '输入或点击录音...'}">
            </div>
            <div class="voice-buttons">
                <button class="voice-record-btn" onclick="startMockVoiceRecord('${config.key}')">
...
function startMockVoiceRecord(key) {
    ...
    btn.innerHTML = '<i class="fas fa-circle" style="color:#E53935;animation:pulse 1s infinite;"></i><span>录音中...</span>';
...
function renderBreathingExercise(duration) {
    ...
    container.innerHTML += `
        <div class="breath-exercise">
            ...
            <p class="breath-timer" id="breathTimer">剩余时间：${Math.floor(duration / 60)}:${(duration % 60).toString().padStart(2, '0')}</p>
        </div>
    `;
...
function startBreathingAnimation(duration) {
    ...
    breathInterval = setInterval(() => {
        ...
        if (!circle) { clearInterval(breathInterval); return; }
    ...
    const timerInterval = setInterval(() => {
        ...
        if (timer) {
            timer.textContent = `剩余时间：${Math.floor(remaining / 60)}:${(remaining % 60).toString().padStart(2, '0')}`;
        }
...
function completeWeek(weekNum) {
    ...
    // 设置作业
    if (weekNum === 1) {
        state.homework = { icon: 'wind', title: '呼吸训练', desc: '每天 10-20 分钟' };
        state.homeworkProgress = 0;
    } else if (weekNum === 2) {
        state.homework = { icon: 'list-check', title: '执行行动计划', desc: '按计划执行并记录' };
        state.homeworkProgress = 0;
    }
...
function selectMood(el) {
    ...
    document.getElementById('homePage').style.background = `linear-gradient(180deg, ${colors[mood]} 0%, #F5F9F5 100%)`;
...
function renderAssessments() {
    ...
    container.innerHTML = PM_DATA.assessments.map(a => `
        <div class="assess-card" onclick="startAssessment('${a.id}')">
            <div class="assess-banner" style="background: ${a.tagType === 'free' ? '#E8F5E9' : '#FFF3E0'}">${a.emoji}</div>
...
const ASSESSMENT_DATA = {
    mbti: {
        title: '16型人格测试',
        questions: [
            { q: '在社交场合中，你通常会：', a: '主动与新朋友交谈', b: '等待别人来找你聊天' },
            { q: '当你在做决定时，你更看重：', a: '逻辑和客观分析', b: '个人价值观和感受' },
            { q: '你更喜欢的生活方式是：', a: '有计划、有条理的', b: '灵活、随性的' },
            { q: '在获取信息时，你更关注：', a: '具体的事实和细节', b: '整体的概念和可能性' },
            { q: '周末时，你更愿意：', a: '参加朋友聚会', b: '独处或与少数亲密朋友在一起' }
        ],
        results: {
            AAAA: { type: 'ESTJ', title: '总经理型', desc: '你是天生的组织者和领导者，善于制定计划并确保执行。' },
            ABAB: { type: 'INFP', title: '调停者型', desc: '你是理想主义者，富有同情心，追求内心的和谐与意义。' },
            BBBB: { type: 'INFP', title: '调停者型', desc: '你是理想主义者，富有同情心，追求内心的和谐与意义。' },
            default: { type: 'ENFP', title: '竞选者型', desc: '你是热情洋溢的创意者，善于激励他人，追求新可能。' }
        }
    },
    sas: {
        title: '焦虑自评量表 (SAS)',
        questions: [
            { q: '我觉得比平常容易紧张和着急', opts: ['没有或很少时间', '小部分时间', '相当多时间', '绝大部分或全部时间'] },
            { q: '我无缘无故地感到害怕', opts: ['没有或很少时间', '小部分时间', '相当多时间', '绝大部分或全部时间'] },
            { q: '我容易心里烦乱或感到惊恐', opts: ['没有或很少时间', '小部分时间', '相当多时间', '绝大部分或全部时间'] },
            { q: '我觉得我可能将要发疯', opts: ['没有或很少时间', '小部分时间', '相当多时间', '绝大部分或全部时间'] },
            { q: '我手脚发抖打颤', opts: ['没有或很少时间', '小部分时间', '相当多时间', '绝大部分或全部时间'] }
        ],
        scoring: (answers) => {
            const total = answers.reduce((sum, a) => sum + a + 1, 0);
            const standardScore = Math.round(total * 1.25 * 4);
            if (standardScore < 50) return { level: '正常', color: '#4CAF50', desc: '你的焦虑水平在正常范围内。' };
            if (standardScore < 60) return { level: '轻度焦虑', color: '#FF9800', desc: '你可能存在轻度焦虑，建议进行放松训练。' };
            if (standardScore < 70) return { level: '中度焦虑', color: '#F57C00', desc: '你存在中度焦虑，建议寻求专业帮助。' };
            return { level: '重度焦虑', color: '#E53935', desc: '你的焦虑水平较高，强烈建议寻求专业心理咨询。' };
        }
    },
    phq9: {
        title: '抑郁自评量表 (PHQ-9)',
        questions: [
            { q: '做事时提不起劲或没有兴趣', opts: ['完全不会', '好几天', '一半以上的天数', '几乎每天'] },
            { q: '感到心情低落、沮丧或绝望', opts: ['完全不会', '好几天', '一半以上的天数', '几乎每天'] },
            { q: '入睡困难、睡不安稳或睡眠过多', opts: ['完全不会', '好几天', '一半以上的天数', '几乎每天'] },
            { q: '感觉疲倦或没有活力', opts: ['完全不会', '好几天', '一半以上的天数', '几乎每天'] },
            { q: '食欲不振或吃太多', opts: ['完全不会', '好几天', '一半以上的天数', '几乎每天'] }
        ],
        scoring: (answers) => {
            const total = answers.reduce((sum, a) => sum + a, 0);
            if (total < 5) return { level: '无抑郁', color: '#4CAF50', desc: '你目前没有明显的抑郁症状。' };
            if (total < 10) return { level: '轻度抑郁', color: '#FF9800', desc: '你可能存在轻度抑郁，建议关注情绪变化。' };
            if (total < 15) return { level: '中度抑郁', color: '#F57C00', desc: '你存在中度抑郁，建议寻求专业帮助。' };
            return { level: '重度抑郁', color: '#E53935', desc: '你的抑郁水平较高，强烈建议寻求专业心理咨询。' };
        }
    },
    burnout: {
        title: '职业倦怠评估',
        questions: [
            ...
            { q: '早上起床想到要上班就感到疲惫', opts: ['从不', '偶尔', '经常', '总是'] },
        ...
        scoring: (answers) => {
            const total = answers.reduce((sum, a) => sum + a, 0);
            if (total < 5) return { level: '无倦怠', color: '#4CAF50', desc: '你目前工作状态良好！' };
            if (total < 10) return { level: '轻度倦怠', color: '#FF9800', desc: '注意工作与生活平衡。' };
            if (total < 15) return { level: '中度倦怠', color: '#F57C00', desc: '建议适当休息和调整。' };
            return { level: '重度倦怠', color: '#E53935', desc: '强烈建议寻求帮助或考虑调整。' };
        }
    }
};
...
function startAssessment(id) {
    ...
    if (!data) { showToast('测评暂不可用'); return; }
...
function showAssessmentSheet() {
    ...
    const progress = ((currentQuestionIndex + 1) / data.questions.length * 100).toFixed(0);
    
    let optionsHtml = '';
    if (currentAssessment.id === 'mbti') {
        optionsHtml = `
            <div class="assess-options">
                <div class="assess-opt" onclick="answerAssessment('a')">${q.a}</div>
                <div class="assess-opt" onclick="answerAssessment('b')">${q.b}</div>
            </div>
        `;
    } else {
        optionsHtml = `
            <div class="assess-options">
                ${q.opts.map((opt, i) => `<div class="assess-opt" onclick="answerAssessment(${i})">${opt}</div>`).join('')}
            </div>
        `;
    }
    ...
    sheet.innerHTML = `
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <h3 style="font-size: 16px; font-weight: 600;">${data.title}</h3>
            <span style="font-size: 13px; color: var(--text-muted);">${currentQuestionIndex + 1}/${data.questions.length}</span>
        </div>
        <div class="progress-bar" style="margin-bottom: 24px;">
            <div class="progress-fill" style="width: ${progress}%; background: var(--primary);"></div>
        </div>
        <p style="font-size: 16px; line-height: 1.6; margin-bottom: 24px;">${q.q}</p>
        ${optionsHtml}
    `;
    ...
    overlay.onclick = () => {
        if (confirm('确定要退出测评吗？')) closeSheet();
    };
}
...
function showAssessmentResult() {
    ...
    if (id === 'mbti') {
        ...
        resultHtml = `
            <div style="text-align: center; padding: 20px 0;">
                ...
                <h2 style="font-size: 28px; color: var(--primary); margin-bottom: 8px;">${result.type}</h2>
                <p style="font-size: 18px; font-weight: 600; margin-bottom: 16px;">${result.title}</p>
                <div class="blur-content" style="${isPro ? '' : 'filter: blur(4px); user-select: none; opacity: 0.6;'}">
                    <p style="font-size: 14px; color: var(--text-secondary); line-height: 1.6;">${result.desc}</p>
                </div>
            </div>
        `;
    } else {
        ...
        resultHtml = `
            <div style="text-align: center; padding: 20px 0;">
                <div style="width: 100px; height: 100px; border-radius: 50%; background: ${result.color}20; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
                    <span style="font-size: 24px; font-weight: 700; color: ${result.color};">${result.level}</span>
                </div>
                <div class="blur-content" style="${isPro ? '' : 'filter: blur(5px); user-select: none; opacity: 0.5;'}">
                    <p style="font-size: 14px; color: var(--text-secondary); line-height: 1.6; margin-bottom: 20px;">${result.desc}</p>
                    <h4 style="text-align:left; margin-top:20px;">📋 专家干预建议：</h4>
                    <p style="text-align:left; font-size: 13px; color: var(--text-muted); margin-top:8px;">根据您的测评结果，我们强烈建议您开始 PM+ 结构化干预课程...</p>
                </div>
            </div>
        `;
    }
    ...
    PM_DATA.state.assessmentHistory.push({
        id, title: data.title, answers: assessmentAnswers, timestamp: Date.now(),
        result: id === 'mbti' ? (data.results[assessmentAnswers.map(a => a.toUpperCase()).join('')] || data.results.default) : data.scoring(assessmentAnswers)
    });
    ...
    if (!isPro) {
        actionButtons = `
            <div style="background: linear-gradient(to bottom, transparent, white 20%); position: absolute; bottom: 0; left: 0; right: 0; padding: 40px 20px 20px; text-align: center; border-radius: 0 0 16px 16px;">
                <p style="font-size: 13px; color: #666; margin-bottom: 15px;">解锁详细解读报告 & 个性化建议</p>
                <button class="btn btn-primary" onclick="showPayModal('report')">解锁深度报告 ¥9.9</button>
                <button class="btn btn-secondary" style="margin-top: 10px;" onclick="closeSheet()">稍后再看</button>
            </div>
        `;
    } else {
        actionButtons = `
            <div style="margin-top: 20px;">
                <button class="btn btn-success" style="width:100%; background:linear-gradient(135deg, #FF6B6B 0%, #EE5253 100%); border:none; color:white; padding:12px; border-radius:12px; font-weight:600;" onclick="showPayModal('pm')">
                    🚀 立即开启 PM+ 系统干预 (半价)
                </button>
                <button class="btn btn-secondary" style="margin-top: 12px; width:100%;" onclick="closeSheet()">返回</button>
            </div>
        `;
    }
    ...
    sheet.innerHTML = `
        <div class="sheet-handle"></div>
        <h3 class="sheet-title">测评初步结果</h3>
...
function renderEnergyTools() {
    ...
    container.innerHTML = PM_DATA.energyTools.map(t => `
...
function startToolBreathing() {
    ...
    function cycle() {
        ...
        circle.style.transform = phase === 0 ? 'scale(1.5)' : phase === 2 ? 'scale(1)' : 'scale(1.5)';
...
let groundingStep = 5;
const groundingData = [
    { num: 5, emoji: '👀', title: '说出你看到的 5 样东西', hint: '环顾四周，专注观察细节' },
    { num: 4, emoji: '👂', title: '说出你听到的 4 种声音', hint: '安静下来，仔细倾听周围' },
    { num: 3, emoji: '✋', title: '触摸 3 样你能碰到的东西', hint: '感受不同的材质和温度' },
    { num: 2, emoji: '👃', title: '闻一闻 2 种气味', hint: '深呼吸，用心感受空气' },
    ...
    const timer = setInterval(() => {
        ...
        if (count <= 0) {
            if (isTense) {
                ...
                document.getElementById('muscleInstruction').textContent = '慢慢放松，感受紧张消散...';
            } else {
                clearInterval(timer);
                muscleIndex++;
            ...
            document.getElementById('muscleTimer').textContent = (isTense ? '紧张 ' : '放松 ') + count;
        }
    }, 1000);
}
...
function nextImageryStep() {
    ...
    if (window.currentImagery.stepIndex >= script.steps.length) {
        document.getElementById('imageryText').innerHTML = '🌟 冥想结束，慢慢睁开眼睛，回到当下。';
        document.querySelector('#sheet .btn-primary').textContent = '完成';
        document.querySelector('#sheet .btn-primary').onclick = closeSheet;
        return;
    }
    
    document.getElementById('imageryText').textContent = script.steps[window.currentImagery.stepIndex];
}
...
function selectJournalMood(el, mood) {
    document.querySelectorAll('#journalMoodGrid .mood-item').forEach(m => m.classList.remove('selected'));
    el.classList.add('selected');
...
function loadJournalHistory() {
    ...
    if (entries.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: var(--text-muted); font-size: 13px;">暂无日记记录</p>';
        return;
    }
    
    const moodEmojis = { happy: '😊', calm: '😌', sad: '😔', anxious: '😟', angry: '😤', neutral: '😐' };
    ...
    container.innerHTML = `
        ...
        ${recent.map(e => `
            <div style="background: var(--bg-secondary); padding: 12px; border-radius: 12px; margin-bottom: 8px;">
                ...
                ${e.text ? `<p style="font-size: 13px; color: var(--text-secondary);">${e.text.substring(0, 50)}${e.text.length > 50 ? '...' : ''}</p>` : ''}
            </div>
        `).join('')}
    `;
}
...
function renderUserProfile() {
    ...
    document.getElementById('userProfile').innerHTML = `
        <div class="profile-header">
            ...
            <p class="profile-days">已在森境旅行 ${Math.floor((Date.now() - (localStorage.getItem('pm_start_time') || Date.now())) / 86400000) || 1} 天</p>
        </div>
        ...
        <div class="profile-stats">
            ...
            <div class="stat-card">
                <p class="stat-value">${state.moodLog.length + (state.journalEntries?.length || 0)}</p>
...
function showGrowthReport() {
    ...
    const state = PM_DATA.state;
    const completedWeeks = state.weekCompleted.filter(w => w).length;
    const preScore = state.psychlops.pre.Q1_Score ? calculatePsychlopsScore(state.psychlops.pre) : null;
        ...
        <div style="padding: 20px 0;">
            ...
            <div style="background: linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%); border-radius: 16px; padding: 20px; color: white; margin-bottom: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div style="font-size: 48px;">${completedWeeks === 5 ? '🎓' : '📚'}</div>
                </div>
                <div class="progress-bar" style="margin-top: 16px; background: rgba(255,255,255,0.2);">
                    <div class="progress-fill" style="width: ${completedWeeks * 20}%; background: var(--accent);"></div>
                </div>
            </div>
            ...
            <div style="background: var(--bg-secondary); border-radius: 16px; padding: 20px; margin-bottom: 20px;">
                ...
                <p style="text-align: center; margin-top: 12px; font-size: 12px; color: var(--success);">
                    ${improvement > 0 ? `📉 困扰程度降低了 ${improvement} 分` : '继续保持，变化正在发生'}
                </p>
            </div>
            ` : ''}
            ...
            <div style="background: var(--bg-secondary); border-radius: 16px; padding: 20px; margin-bottom: 20px;">
                ...
                <div style="display: flex; justify-content: space-around;">
                    ${Object.entries(moodStats).map(([mood, count]) => `
                        <div style="text-align: center;">
                            <p style="font-size: 24px;">${getMoodEmoji(mood)}</p>
            ...
            <div style="background: var(--bg-secondary); border-radius: 16px; padding: 20px;">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                    ${state.weekCompleted[0] ? '<span style="background: var(--primary); color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px;">呼吸训练</span>' : ''}
                    ${state.weekCompleted[1] ? '<span style="background: var(--primary); color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px;">问题管理</span>' : ''}
                    ${state.weekCompleted[2] ? '<span style="background: var(--primary); color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px;">行为激活</span>' : ''}
                    ${state.weekCompleted[3] ? '<span style="background: var(--primary); color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px;">社会支持</span>' : ''}
                    ${state.weekCompleted[4] ? '<span style="background: var(--primary); color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px;">预防复发</span>' : ''}
                    ${completedWeeks === 0 ? '<span style="color: var(--text-muted); font-size: 13px;">完成课程后解锁</span>' : ''}
                </div>
            </div>
        </div>
...
function getMoodStats() {
    ...
    moods.forEach(m => { if (stats[m.mood] !== undefined) stats[m.mood]++; });
    return stats;
}

function getMoodEmoji(mood) {
    const emojis = { happy: '😊', calm: '😌', sad: '😔', anxious: '😟', angry: '😤' };
    return emojis[mood] || '😐';
}
...
function showSessionDetail(week) {
    const sheet = document.getElementById('sheet');
    const messages = (PM_DATA.state.chatHistory || []).filter(msg => msg.week === week);
    const weekData = PM_DATA.weeks[week - 1];
    
    // 生成简化的聊天界面视图
    const chatHtml = messages.map(msg => `
        <div class="bubble ${msg.role}" style="font-size: 14px; padding: 12px 16px; margin-bottom: 12px; max-width: 100%; box-shadow: none; border: 1px solid ${msg.role === 'ai' ? '#eee' : 'transparent'};">
            ${msg.content}
        </div>
    `).join('');
    ...
    sheet.innerHTML = `
        ...
        <div style="display:flex; align-items:center; gap:8px; margin-bottom:16px;">
            ...
            <h3 class="sheet-title" style="margin:0; flex:1; text-align:left; font-size: 18px;">第 ${week} 周: ${weekData.title}</h3>
        </div>
        
        <div style="height: 400px; overflow-y: auto; background: #F8FAF8; padding: 16px; border-radius: 16px; margin-bottom: 16px;">
            ${messages.length > 0 ? chatHtml : '<p style="text-align: center; color: var(--text-muted); font-size: 13px; margin-top: 40px;">暂无该周的详细记录</p>'}
        </div>
            ...
            ` : history.map(h => `
                <div style="padding: 16px; background: var(--bg-secondary); border-radius: 16px; margin-bottom: 12px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        ...
                        <span style="font-size: 12px; color: var(--text-muted);">${new Date(h.timestamp).toLocaleDateString()}</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <span style="padding: 4px 10px; border-radius: 12px; font-size: 12px; background: ${h.result.color || 'var(--primary)'}20; color: ${h.result.color || 'var(--primary)'};">
    ...
    sheet.innerHTML = `
        ...
        <div style="padding: 20px 0;">
            <div style="background: linear-gradient(135deg, #FFD700 0%, #FFA000 100%); border-radius: 20px; padding: 24px; color: white; text-align: center; margin-bottom: 24px;">
                ...
                <p style="font-size: 13px; opacity: 0.95;">无限次专业测评 · 无限次PM+ · 1对1专属客服</p>
            </div>
            
            <div style="margin-bottom: 20px;">
                <div style="display: flex; align-items: center; gap: 12px; padding: 14px 0; border-bottom: 1px solid var(--border-light);">
                    <i class="fas fa-check-circle" style="color: var(--success);"></i>
                    <span>解锁所有付费专业量表</span>
                </div>
                <div style="display: flex; align-items: center; gap: 12px; padding: 14px 0; border-bottom: 1px solid var(--border-light);">
                    <i class="fas fa-check-circle" style="color: var(--success);"></i>
                    <span>AILING 陪伴无限制使用</span>
                </div>
                <div style="display: flex; align-items: center; gap: 12px; padding: 14px 0; border-bottom: 1px solid var(--border-light);">
                    <i class="fas fa-check-circle" style="color: var(--success);"></i>
                    <span>PM+ 一年内无限次使用</span>
                </div>
            ...
            <div style="display: flex; gap: 12px; margin-bottom: 20px;">
                <div style="flex: 1; background: var(--bg-secondary); border-radius: 16px; padding: 20px; text-align: center; border: 2px solid transparent;" onclick="selectPlan(this)">
                    <p style="font-size: 12px; color: var(--text-muted);">单次购买PM+</p>
                    <p style="font-size: 24px; font-weight: 700; color: var(--primary);">¥199<span style="font-size: 12px;">/次</span></p>
                </div>
                <div style="flex: 1; background: var(--bg-secondary); border-radius: 16px; padding: 20px; text-align: center; border: 2px solid var(--primary); position: relative;" onclick="selectPlan(this)">
                    <span style="position: absolute; top: -10px; right: 10px; background: var(--accent); color: white; font-size: 10px; padding: 2px 8px; border-radius: 10px;">推荐</span>
                    <p style="font-size: 12px; color: var(--text-muted);">年度会员</p>
                    <p style="font-size: 24px; font-weight: 700; color: var(--primary);">¥1280<span style="font-size: 12px;">/年</span></p>
                    <p style="font-size: 11px; color: var(--success);">省 44%</p>
                </div>
            </div>
        </div>
        
        <button class="btn btn-primary" onclick="showToast('支付功能开发中'); closeSheet();">立即开通</button>
    `;
...
function showPsychlopsDetail() {
    const score = PM_DATA.state.psychlops.pre.Q1_Score ? calculatePsychlopsScore(PM_DATA.state.psychlops.pre) : null;
    if (!score) {
        showToast('完成 PM+ 预评估后查看详情');
        return;
    }
    showModal('success', '📊', `PSYCHLOPS: ${score}`, 
        `这是你的心理困扰基准分数。分数越低表示状况越好。完成PM+课程后会有明显改善。`, null);
}
...
function showToast(message) {
    ...
    toast.style.cssText = 'position: fixed; top: 100px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.8); color: white; padding: 12px 24px; border-radius: 24px; font-size: 14px; z-index: 300; animation: fadeIn 0.3s;';
...
function showCompletionSummary() {
    showModal('success', '🎓', 'PM+ 已完成', '你已经完成了全部5周课程，继续使用能量坊工具保持心理健康！', null);
}

// AiLing 聊天
const AI_RESPONSES = {
    '心情': ['我听到你现在心情可能不太好。能告诉我具体发生了什么吗？', '心情低落是很正常的感受。你愿意多说一些吗？', '每个人都会有情绪起伏。你现在最困扰的是什么？'],
    '压力': ['压力是现代生活中很常见的问题。你最近是什么让你感到压力呢？', '听起来你承受了不少压力。要不要试试我们的呼吸练习来放松一下？', '压力过大时，身体和心理都会有反应。你最近睡眠和食欲怎么样？'],
    '放松': ['很棒，学习放松技巧是照顾自己的好方法！你可以去"能量坊"尝试呼吸训练或渐进式肌肉放松。', '我推荐你试试 4-7-8 呼吸法：吸气4秒、屏息7秒、呼气8秒。非常有效！', '着陆技术也是一个好选择：注意5样你看到的、4样你听到的、3样你能触摸的东西...'],
    '睡眠': ['睡眠问题确实会影响我们的状态。你是入睡困难，还是容易早醒呢？', '睡前可以试试渐进式肌肉放松，或者做几分钟呼吸练习。', '建立规律的睡眠习惯很重要。尽量每天同一时间睡觉和起床。'],
    '焦虑': ['焦虑的感觉确实很不舒服。你能描述一下焦虑时有什么身体感受吗？', '当感到焦虑时，可以试试着陆技术，把注意力拉回当下。', '记住，焦虑的情绪会来也会走。我们一起来应对它。'],
    '工作': ['工作上的压力确实会影响心情。现在工作中最让你困扰的是什么？', '在职场中保持边界感很重要。你有尝试过设定工作与生活的边界吗？', '有时候工作压力需要我们主动寻求帮助。你身边有可以倾诉的同事吗？'],
    'default': [
...
function updateMindGraph(animate = false) {
    ...
    if (!PM_DATA.state.mindGraphData) PM_DATA.state.mindGraphData = [50,50,50,50,50,50];
    ...
    // 生成 SVG 坐标 points
    const points = newData.map((val, i) => {
...
function showPayModal(type) {
    const overlay = document.querySelector('.pay-modal-overlay') || createPayModal();
    ...
    if (type === 'report') {
        ...
        desc.textContent = '获取包含多维度性格分析、潜在风险预警及专家定制建议的完整报告。';
        price.innerHTML = '¥9.9 <small>原价 ¥29.9</small>';
        btn.onclick = () => simulatePayment('report');
    } else {
        title.textContent = '开启 PM+ 专业干预疗程';
        desc.textContent = '解锁完整 5 周 WHO 认证循证干预课程，包含所有工具与 AI 陪伴功能。';
        price.innerHTML = '¥199 <small>限时特惠</small>';
        btn.onclick = () => simulatePayment('pm');
    }
...
function simulatePayment(type) {
    ...
    setTimeout(() => {
        ...
        if (type === 'pm') PM_DATA.state.unlockedPM = true;
        ...
        // 刷新状态
        if (type === 'report') showAssessmentResult(); 
        if (type === 'pm') {
            switchPage('pm');
            enterPM();
        }
...
function stopVoiceRecord(e) {
    ...
    // 获取最终文本
    const statusEl = document.getElementById('voiceStatus');
    const text = finalTranscript || (statusEl.innerText !== '正在聆听...' ? statusEl.innerText : '');
